...
// Priority 1: Gmail - EMAIL_USER + EMAIL_PASS (use Gmail App Password). Used when both are set.
// Priority 2: Brevo - BREVO_SMTP_USER + BREVO_SMTP_KEY (SMTP key from Brevo Settings > SMTP & API, NOT API key).
const createTransporter = () => {
  const emailUser = process.env.EMAIL_USER;
  const emailPass = process.env.EMAIL_PASS;

  // Prefer Gmail when EMAIL_USER and EMAIL_PASS are set (e.g. tungananaveenjob18@gmail.com + App Password)
  if (emailUser && emailPass) {
    console.log("📧 [EMAIL] Using Gmail SMTP (EMAIL_USER/EMAIL_PASS)");
  ...
  const brevoPort = parseInt(process.env.BREVO_SMTP_PORT, 10) || 587;
  ...
  if (brevoSMTPKey && brevoUser) {
    ...
    transporter.verify((error) => {
      if (error) {
        console.error(
          "❌ [EMAIL] Brevo SMTP configuration error:",
          error.message,
        );
      } else {
  ...
  console.error(
    "❌ [EMAIL] No email provider configured. Set EMAIL_USER + EMAIL_PASS (Gmail App Password) or BREVO_SMTP_USER + BREVO_SMTP_KEY.",
  );
  return nodemailer.createTransport({
    host: "smtp.gmail.com",
    port: 587,
...
const sendEmail = async (
  ...
  try {
    ...
    if (!to || !subject || (!html && !text)) {
      console.error(
        "❌ [EMAIL] Missing required fields: to, subject, and html/text",
      );
      return {
        success: false,
    ...
    const defaultSenderEmail =
      process.env.SENDER_EMAIL || process.env.EMAIL_USER;
    if (!defaultSenderEmail) {
      console.error("❌ [EMAIL] SENDER_EMAIL not configured");
      return {
        success: false,
        message:
          "Email sender not configured. Please set SENDER_EMAIL in environment variables.",
      };
    }
    ...
    const mailOptions = {
      ...
      ...(fromEmail && fromEmail !== defaultSenderEmail
        ? { replyTo: fromEmail }
        : {}),
    };
    ...
    if (fromEmail && fromEmail !== defaultSenderEmail) {
      console.log(
        `📧 [EMAIL] Using Reply-To: ${fromEmail} (${fromName || "Building Owner"})`,
      );
    ...
    if (error.code === "EAUTH") {
      errorMessage =
        "Email authentication failed. Brevo: use BREVO_SMTP_KEY (SMTP key from Brevo Settings > SMTP & API, not API key). Gmail: use EMAIL_PASS as App Password (Google Account > Security > App passwords).";
    } else if (error.code === "ECONNECTION") {
      errorMessage =
        "Failed to connect to email server. Please check network connection.";
...
const sendOTPEmail = async (
  ...
  console.log(
    `📧 [EMAIL] Preparing OTP email for ${purposeLabels[purpose] || purpose}`,
  );
  console.log(`📧 [EMAIL] Recipient: ${email}`);
  console.log(`📧 [EMAIL] OTP Code: ${otp.toString()}`);
  if (fromEmail) {
    console.log(
      `📧 [EMAIL] Sending from: ${fromEmail} (${fromName || "Building Owner"})`,
    );
  }
  ...
  try {
    ...
    if (result.success) {
      ...
      if (purpose === "admin_registration") {
        console.log(
          `✅ [EMAIL] Admin Registration OTP email delivered to Gmail: ${email}`,
        );
      }
      if (purpose === "resident_creation") {
        console.log(`✅ [EMAIL] Resident creation OTP email sent successfully`);
        console.log(
          `📧 [EMAIL] Reply-To will be: ${fromEmail || "default sender"}`,
        );
      }
    } else {
...
const sendAdminUserCreationOTP = async (adminEmail, otp, userData) => {
  ...
  try {
    const html = await loadEmailTemplate(
      EMAIL_TEMPLATES.ADMIN_USER_CREATION_OTP,
      templateVars,
    );
    console.log(
      `✅ [EMAIL] Admin user creation OTP template loaded successfully`,
    );
    ...
    if (result.success) {
      console.log(
        `✅ [EMAIL] Admin OTP email sent successfully to ${adminEmail}`,
      );
    } else {
      console.error(
        `❌ [EMAIL] Admin OTP email sending failed to ${adminEmail}`,
      );
      console.error(`❌ [EMAIL] Error: ${result.message}`);
    }
    ...
    console.error(
      `❌ [EMAIL] Error in sendAdminUserCreationOTP:`,
      error.message,
    );
    return {
      success: false,
...
const sendAccountConfirmationEmail = async (user) => {
  ...
  try {
    ...
    console.log(
      `✅ [EMAIL] Account confirmation email template loaded successfully`,
    );

    const subject = `Welcome to ApartmentSync - Your Account is Ready!`;
    console.log(
      `📧 [EMAIL] Sending account confirmation email to ${user.email}`,
    );
    ...
    if (result.success) {
      console.log(
        `✅ [EMAIL] Account confirmation email sent successfully to ${user.email}`,
      );
    } else {
      console.error(
        `❌ [EMAIL] Account confirmation email sending failed to ${user.email}`,
      );
      console.error(`❌ [EMAIL] Error: ${result.message}`);
    }
    ...
    console.error(
      `❌ [EMAIL] Error in sendAccountConfirmationEmail:`,
      error.message,
    );
    return {
      success: false,
...
const sendAdminNewUserNotification = async (
  adminEmail,
  user,
  ...
  try {
    const html = await loadEmailTemplate(
      EMAIL_TEMPLATES.ADMIN_NEW_USER_NOTIFICATION,
      templateVars,
    );
    ...
    if (result.success) {
      console.log(
        `✅ [EMAIL] Admin notification sent successfully to ${adminEmail}`,
      );
    } else {
    ...
    console.error(
      `❌ [EMAIL] Error in sendAdminNewUserNotification:`,
      error.message,
    );
    return {
      success: false,
...
const sendUserStatusChangeEmail = async (
  user,
  oldStatus,
  newStatus,
  reason = null,
  ...
  const statusConfig = {
    active: {
      ...
      class: "approved",
      message:
        "Your account is now active. You can access all features of ApartmentSync.",
      bgColor: "#dcfce7",
      textColor: "#166534",
    },
    approved: {
  ...
  const config =
    statusConfig[newStatus.toLowerCase()] || statusConfig["pending"];
  ...
  try {
    const html = await loadEmailTemplate(
      EMAIL_TEMPLATES.USER_STATUS_CHANGE,
      templateVars,
    );
    ...
    if (result.success) {
      console.log(
        `✅ [EMAIL] Status change email sent successfully to ${user.email}`,
      );
    } else {
    ...
    console.error(
      `❌ [EMAIL] Error in sendUserStatusChangeEmail:`,
      error.message,
    );
    return {
      success: false,
...
const sendAccountCreatedEmail = async (user, otp) => {
  ...
  try {
    ...
    console.log(
      `✅ [EMAIL] Account created email template loaded successfully`,
    );

    const subject = `Welcome to ApartmentSync - Verify Your Email`;
    console.log(
      `📧 [EMAIL] Sending account creation email with OTP to ${user.email}`,
    );
    ...
    if (result.success) {
      console.log(
        `✅ [EMAIL] Account creation email sent successfully to ${user.email}`,
      );
    } else {
      console.error(
        `❌ [EMAIL] Account creation email sending failed to ${user.email}`,
      );
      console.error(`❌ [EMAIL] Error: ${result.message}`);
    }
    ...
    console.error(
      `❌ [EMAIL] Error in sendAccountCreatedEmail:`,
      error.message,
    );
    console.error(`❌ [EMAIL] Error Stack:`, error.stack);
    return {
...
const sendComplaintRegisteredEmail = async (user, complaint) => {
  ...
  const html = await loadEmailTemplate(
    EMAIL_TEMPLATES.COMPLAINT_REGISTERED,
    templateVars,
  );
...
const sendComplaintStatusUpdateEmail = async (
  ...
  const html = await loadEmailTemplate(
    EMAIL_TEMPLATES.COMPLAINT_STATUS_UPDATE,
    templateVars,
  );
...
const sendComplaintResolvedEmail = async (user, complaint) => {
  const templateVars = {
    ...
    resolutionDescription:
      complaint.resolution?.description || "Issue has been resolved",
  ...
  const html = await loadEmailTemplate(
    EMAIL_TEMPLATES.COMPLAINT_RESOLVED,
    templateVars,
  );
...
const sendNoticePublishedEmail = async (users, notice) => {
  ...
  const html = await loadEmailTemplate(
    EMAIL_TEMPLATES.NOTICE_PUBLISHED,
    templateVars,
  );
...
const sendAccountApprovedEmail = async (user) => {
  ...
  const html = await loadEmailTemplate(
    EMAIL_TEMPLATES.ACCOUNT_APPROVED,
    templateVars,
  );
...
const sendSecurityAlertEmail = async (user, alertType, metadata = {}) => {
  ...
  return await sendEmail(user.email, "Security Alert - ApartmentSync", html);
};
...
const sendVisitorEntryEmail = async (hostResident, visitorData) => {
  ...
  const entryDate = visitorData.entryDate
    ? new Date(visitorData.entryDate).toLocaleString("en-IN", {
        timeZone: "Asia/Kolkata",
      })
    : new Date().toLocaleString("en-IN", { timeZone: "Asia/Kolkata" });

  const expectedCheckOutTime = visitorData.expectedCheckOutTime
    ? new Date(visitorData.expectedCheckOutTime).toLocaleString("en-IN", {
        timeZone: "Asia/Kolkata",
      })
    : null;
  ...
  const templateVars = {
    ...
    flatNumber:
      ...
      `${hostResident.wing}-${hostResident.flatNumber}`,
...
const sendVisitorCheckInEmail = async (hostResident, visitorData) => {
  ...
  const checkInTime = visitorData.checkInTime
    ? new Date(visitorData.checkInTime).toLocaleString("en-IN", {
        timeZone: "Asia/Kolkata",
      })
    : new Date().toLocaleString("en-IN", { timeZone: "Asia/Kolkata" });
  ...
  const html = await loadEmailTemplate(
    EMAIL_TEMPLATES.VISITOR_CHECKIN,
    templateVars,
  );
...
const sendVisitorStatusUpdateEmail = async (hostResident, visitorData) => {
  ...
  const html = await loadEmailTemplate(
    EMAIL_TEMPLATES.VISITOR_STATUS_UPDATE,
    templateVars,
  );
...
const sendAdminNewComplaintEmail = async (
  adminEmail,
  complaint,
  creator,
  location,
  ...
  try {
    const html = await loadEmailTemplate(
      EMAIL_TEMPLATES.ADMIN_NEW_COMPLAINT,
      templateVars,
    );
    ...
    console.log(
      `📧 [EMAIL] Sending admin new complaint email to ${adminEmail}`,
    );
    ...
    if (result.success) {
      console.log(
        `✅ [EMAIL] Admin new complaint email sent successfully to ${adminEmail}`,
      );
    } else {
      console.error(
        `❌ [EMAIL] Admin new complaint email failed: ${result.message}`,
      );
    }
    ...
    console.error(
      `❌ [EMAIL] Error in sendAdminNewComplaintEmail:`,
      error.message,
    );
    return {
      success: false,
...
const sendAdminComplaintStatusChangeEmail = async (
  adminEmail,
  complaint,
  oldStatus,
  newStatus,
  ...
  try {
    const html = await loadEmailTemplate(
      EMAIL_TEMPLATES.ADMIN_COMPLAINT_STATUS_CHANGE,
      templateVars,
    );
    console.log(
      `✅ [EMAIL] Admin complaint status change email template loaded`,
    );

    const subject = `Complaint Status Updated - Ticket #${complaint.ticketNumber}`;
    console.log(
      `📧 [EMAIL] Sending admin status change email to ${adminEmail}`,
    );
    ...
    if (result.success) {
      console.log(
        `✅ [EMAIL] Admin status change email sent successfully to ${adminEmail}`,
      );
    } else {
      console.error(
        `❌ [EMAIL] Admin status change email failed: ${result.message}`,
      );
    }
    ...
    console.error(
      `❌ [EMAIL] Error in sendAdminComplaintStatusChangeEmail:`,
      error.message,
    );
    return {
      success: false,
...
const sendVisitorCheckOutEmail = async (hostResident, visitorData) => {
  ...
  const checkOutTime = visitorData.checkOutTime
    ? new Date(visitorData.checkOutTime).toLocaleString("en-IN", {
        timeZone: "Asia/Kolkata",
      })
    : new Date().toLocaleString("en-IN", { timeZone: "Asia/Kolkata" });
  ...
  const html = await loadEmailTemplate(
    EMAIL_TEMPLATES.VISITOR_CHECKOUT,
    templateVars,
  );
