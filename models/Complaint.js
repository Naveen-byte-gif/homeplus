...
const complaintSchema = new mongoose.Schema({
  ...
  status: {
    type: String,
    enum: ['Open', 'Assigned', 'In Progress', 'Resolved', 'Closed', 'Cancelled', 'Reopened'],
    default: 'Open'
  },
  location: {
...
complaintSchema.pre('save', async function(next) {
  if (this.isNew) {
    ...
    this.ticketNumber = `APT-${Date.now().toString().slice(-6)}-${(count + 1).toString().padStart(4, '0')}`;
    ...
    this.sla.expectedResolution = new Date(Date.now() + slaHours[this.priority] * 60 * 60 * 1000);
  }
  next();
});

// Instance method to update status with timeline tracking
complaintSchema.methods.updateStatus = async function(newStatus, description, updatedBy) {
  ...
  if (newStatus === 'Resolved') {
    ...
    this.sla.isBreached = this.sla.actualResolution > this.sla.expectedResolution;
  }
